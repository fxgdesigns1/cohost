...
from app.datastore import (
    ...
    delete_draft,
    log_message,
    upsert_thread_marker,
)


class Settings(BaseSettings):
    # Important: this must match your Cloud Run HTTPS URL + /oauth2/callback/google
    OAUTH_REDIRECT_URI: str = "http://localhost:8000/oauth2/callback/google"

    # Optional: only used in local/dev if you load a web client JSON directly
    OAUTH_CLIENT_ID: str = ""
...
def oauth_start(hostId: str):
    if not get_tenant(hostId):
        raise HTTPException(
            status_code=400,
            detail="Unknown hostId. Call /tenants/register first.",
        )

    # Build flow using the secret mounted at /secrets/client_secret.json by default
    secret_path = os.getenv(
        "GOOGLE_CLIENT_SECRET_PATH", "/secrets/client_secret.json"
    )
    flow = oauth_flow(secret_path, settings.OAUTH_REDIRECT_URI)
...
def oauth_callback(request: Request):
    ...
    if not state.startswith("host:") or not code:
        return JSONResponse(
            {"ok": False, "error": "bad_state_or_code"}, status_code=400
        )

    host_id = state.split("host:", 1)[1]
    if not get_tenant(host_id):
        return JSONResponse(
            {"ok": False, "error": "unknown_host"}, status_code=400
        )
